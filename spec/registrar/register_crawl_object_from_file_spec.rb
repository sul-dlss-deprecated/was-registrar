...
describe Was::Registrar::RegisterCrawlObjectFromFile do
  ...
  describe '.verify_file' do
    ...
    it 'returns false and print error message if the file is bad-formatted with one line' do
      ...
      expect(STDOUT).to receive(:puts).with("Problem in line WAS:Test\tjob1/warc\tdruid:ab123cd4567a")
    ...
    it 'returns false and print error message if the file is bad-formatted with multi lines' do
      ...
      expect(STDOUT).to receive(:puts).with("Problem in line WAS:Test\tjob1/warc\tdruid:ab123cd4567a")
      expect(STDOUT).to receive(:puts).with("Problem in line WAS:Test\t\tdruid:ef123gh4567")
  ...
  describe '.register' do
    it 'register one item in the crawl list file with one item' do
      ...
      expect_any_instance_of(Was::Registrar::RegisterCrawlObject).to receive(:register).with('source_id' => 'WAS:Test', 'job_directory' => 'job1/warc', 'collection_id' => 'druid:ab123cd4567').and_return('druid:ab123cd4567')
    ...
    it 'registers items in the crawl list file with multiple lines' do
      valid_file = "#{@fixtures}register_crawl_files/valid_multi_lines.txt"
      allow(Was::Registrar::RegisterCrawlObjectFromFile).to receive(:verify_file).and_return(true)
      expect_any_instance_of(Was::Registrar::RegisterCrawlObject).to receive(:register).once.with('source_id' => 'WAS:Test', 'job_directory' => 'job1/warc', 'collection_id' => 'druid:ab123cd4567').and_return('druid:xy123yz4567')
      expect_any_instance_of(Was::Registrar::RegisterCrawlObject).to receive(:register).once.with('source_id' => 'WAS:Test', 'job_directory' => 'job2/warc', 'collection_id' => 'druid:ef123gh4567').and_return('druid:mn123pq4567')
    ...
    it 'raises an exception that should be logged in the log file' do
      ...
      expect_any_instance_of(Logger).to receive(:fatal).with("Error in registering WAS:Test\tjob1/warc\tdruid:ab123cd4567 with #<NoMethodError: undefined method `[]' for nil:NilClass>")
      Was::Registrar::RegisterCrawlObjectFromFile.register(valid_file, 'log/tmp.txt')
    end
  end
end
